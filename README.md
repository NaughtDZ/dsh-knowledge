...
- 📚 **知识库** — 一个知识库包含多个导入文件。
- 🗂 **工作区挂载** — 把不同的知识库**选择性地启用**到不同工作区。
- 🤖 **agent 工具** — 提供 `knowledge_search` 工具，agent 自动检索当前/指定工作区已启用的知识库。
- 🎛 **左侧边栏入口 + 网页设置页** — 侧边栏底部一个入口按钮，点开是完整的网页管理面板（配置 / 知识库 / 工作区挂载 / 检索测试），也注册了 Settings 页面。
...
1. **打开面板** — 点击左侧边栏底部的「知识库」按钮（或 Settings -> 知识库）。
2. **配置嵌入** — 在「配置」页填：OpenAI 兼容接口地址、嵌入模型、可选 API Key、可选重排模型；点「测试连接」验证。
3. **创建知识库并导入文件** — 「知识库」页新建一个库，选择一个库后「导入文件」（支持多选/拖拽，浏览器文件选择）。
4. **挂载到工作区** — 「知识库」页底部「工作区挂载」：为一个工作区启用若干知识库。
5. **agent 检索** — 模型在运行时自动拥有 `knowledge_search` 工具；你也可以在「检索」页手动测试。
