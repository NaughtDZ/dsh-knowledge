 ...
 */
export function migrateLegacyDataDir(canonical: string): void {
  try {
    const legacy = join(process.cwd(), 'dsh-knowledge')
    if (legacy === canonical) return
    if (existsSync(join(canonical, 'knowledge.sqlite'))) return
    if (!existsSync(join(legacy, 'knowledge.sqlite'))) return
