...
import { embedBatched, embedTexts, type EmbedConfig } from './embed.ts'
...
/** Mutable internal operation state (never sent over the wire directly). */
...
export class KnowledgeEngine {
  ...
  deleteBase(id: string): void {
    ...
    for (const file of this.store.filesOf(id)) this.removeFileFromDisk(file.relPath)
    ...
    if (this.filesRoot !== undefined) {
      try { rmSync(join(this.filesRoot, id), { recursive: true, force: true }) } catch { /* best effort */ }
    }
  }
  ...
  deleteFile(id: string): void {
    ...
    if (file !== null) this.removeFileFromDisk(file.relPath)
  ...
  private viewOf(op: ActiveOperation): OperationView {
  ...
  stopOperation(id: string): boolean {
    ...
    if (op === undefined || op.finished) return false
    op.controller.abort()
    return true
  }
  ...
  stopAll(): number {
    ...
    for (const op of [...this.operations.values()]) {
      if (!op.finished) { op.controller.abort(); count++ }
    }
    return count
  }
   ...
   */
  async reembedFile(fileId: string, signal?: AbortSignal, onProgress?: (current: string, chunkTotal: number, chunkDone: number) => void): Promise<ImportResult | null> {
    const file = this.store.fileOf(fileId)
    if (file === null) return null
    const bytes = this.readFileBytes(file.relPath)
    if (bytes === null) {
      this.store.updateFile(fileId, { status: 'failed', error: 'original file is missing on disk' } as Partial<KnowledgeFile>)
      return null
    }
    this.store.updateFile(fileId, { status: 'indexing' })
    try {
      ...
      this.store.updateFile(fileId, { status: 'failed', error: stopped ? 'stopped by user' : (error instanceof Error ? error.message : String(error)) } as Partial<KnowledgeFile>)
      return null
    }
  }
   ...
   */
  private async indexBytes(fileId: string, kbId: string, filename: string, bytes: Buffer, signal?: AbortSignal, onProgress?: (current: string, chunkTotal: number, chunkDone: number) => void): Promise<ImportResult> {
    const config = this.store.getConfig()
    ...
    if (aborted(signal)) throw new Error(STOPPED)
    const embedConfig = this.embedConfig(config)
    ...
    if (aborted(signal) || vectors.length < texts.length) throw new Error(STOPPED)
    ...
    const chunkRecords: Array<Pick<Chunk, 'index' | 'text' | 'heading' | 'dim' | 'vector'>> =
      pieces.map((piece, i) => ({ index: i, text: piece.text, heading: piece.heading, dim, vector: vectors[i] ?? [] }))
    const now = Date.now()
  ...
  private async runBatch(op: ActiveOperation, fileIds: string[]): Promise<void> {
    try {
      for (const fileId of fileIds) {
        if (op.controller.signal.aborted) break
        const file = this.store.fileOf(fileId)
  ...
  private track(kind: OperationKind, label: string, total: number): ActiveOperation {
    const id = randomUUID()
  ...
  private removeOp(id: string): void {
    this.operations.delete(id)
  }

  /** Delete a raw uploaded file and prune now-empty parent dirs up to filesRoot. */
  private removeFileFromDisk(relPath: string): void {
    if (this.filesRoot === undefined || relPath === '') return
    const target = join(this.filesRoot, relPath)
    try {
      // Path-traversal guard.
      const rel = relative(this.filesRoot, target)
      if (rel === '' || rel.startsWith('..') || rel.split(/[\\/]/u).includes('..')) return
    ...
    while (dir !== this.filesRoot && dir.startsWith(this.filesRoot)) {
      try {
        if (existsSync(dir) && readdirEmpty(dir)) rmSync(dir, { recursive: false })
        else break
      } catch {
        break
      }
