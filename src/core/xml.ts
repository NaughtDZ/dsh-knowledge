...
function decodeEntities(value: string): string {
  return value
    ...
    .replace(/&quot;/g, '"')
    .replace(/&apos;/g, "'")
    .replace(/&#(\d+);/g, (_m, cp: string) => String.fromCodePoint(Number(cp)))
    .replace(/&#x([0-9a-fA-F]+);/g, (_m, cp: string) => String.fromCodePoint(Number.parseInt(cp, 16)))
}

/** Parse one opening or self-closing tag from its raw text. */
function parseTag(raw: string): LexedTag {
  let i = 1 // skip '<'
...
export function parseXml(source: string): XmlElement | null {
  ...
  function parseElement(): XmlElement {
    ...
    for (;;) {
      ...
      if (source[pos] !== '<') {
        let next = pos
        while (next < len && source[next] !== '<') next++
        const text = source.slice(pos, next)
        pos = next
        if (text.trim() !== '') element.children.push({ kind: 'text', text: decodeEntities(text) })
        continue
      }
  ...
  skipMisc()
  if (pos >= len || source[pos] !== '<') return null
  return parseElement()
}

/** Collect every descendant (including self) element whose local tag matches. */
export function descendants(node: XmlElement, localTag: string): XmlElement[] {
  const out: XmlElement[] = []
  const visit = (n: XmlElement): void => {
    const local = localNameOf(n.tag)
    if (local === localTag) out.push(n)
...
export function localNameOf(tag: string): string {
  const idx = tag.indexOf(':')
  return idx === -1 ? tag : tag.slice(idx + 1)
}

/** Direct child elements of one element. */
export function childrenOf(node: XmlElement): XmlElement[] {
