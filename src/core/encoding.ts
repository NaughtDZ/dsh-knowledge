...
const ENCODINGS_BY_NAME: Record<string, string> = {
  'utf-16le': 'utf-16le',
  'utf-16be': 'utf-16be',
  'utf-32le': 'utf-32le',
  'utf-32be': 'utf-32be',
...
function supports(encoding: string): boolean {
  try {
...
function tryFatal(buffer: Uint8Array, encoding: string): string | null {
  if (!supports(encoding)) return null
  try {
...
function quality(text: string): number {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if ((code < 32 && code !== 9 && code !== 10 && code !== 13) || code === 127) control++
...
function stripBom(buffer: Uint8Array, encoding: string): Uint8Array {
  for (const bom of BOMS) {
    if (bom.encoding === encoding && buffer.length >= bom.bytes.length) {
      ...
      if (match) return buffer.subarray(bom.bytes.length)
    }
  }
  return buffer
}
 ...
 */
export function decodeText(buffer: Uint8Array, preferred?: string): DecodedText {
  // BOM detection.
  for (const bom of BOMS) {
    if (bom.bytes.length > buffer.length) continue
    let match = true
    for (let i = 0; i < bom.bytes.length; i++) {
      if (buffer[i] !== bom.bytes[i]) { match = false; break }
    }
  ...
  let best: { encoding: string; text: string; score: number } | null = null
...
export function looksLikeText(buffer: Uint8Array): boolean {
  ...
  if (buffer.length >= 4) {
    ...
    if (n === 0x04034b50) return false // zip
    if (buffer.length >= 8) {
      const sig = Buffer.from(buffer.subarray(0, 8)).toString('hex')
      if (sig === 'd0cf11e0a1b11ae1') return false // OLE2
      if (sig.startsWith('25504446') || sig.startsWith('%pdf')) return false // PDF
    }
  }
