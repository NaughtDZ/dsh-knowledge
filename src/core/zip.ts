 ...
 */
export function listZipEntries(buf: Buffer): Map<string, { method: number; compressedSize: number; localOffset: number }> {
  ...
  if (offset === 0xffffffff || entryCount === 0xffff) throw new Error('zip: ZIP64 unsupported')
  const out = new Map<string, { method: number; compressedSize: number; localOffset: number }>()
...
export function readZipEntry(buf: Buffer, name: string): Buffer | null {
  ...
  if (sig !== 0x04034b50) throw new Error('zip: bad local header')
...
export function readZipByPrefix(buf: Buffer, prefix: string): Array<{ name: string; data: Buffer }> {
  const entries = listZipEntries(buf)
