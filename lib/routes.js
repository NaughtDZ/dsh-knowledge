import { asObject, errorMessage, pathnameOf, readJsonBody, readRawBody, sameOrigin, sendError, sendJson } from "./http.js";
/** GET `/knowledge/config`. */
function getConfig(engine, res) {
    sendJson(res, 200, engine.getConfig());
}
/** POST `/knowledge/config/{...}` — body patch, or `test` to probe the endpoint. */
async function postConfig(engine, req, res, tail) {
    ...
    if (tail === 'test') {
        ...
        try {
            const { embedText } = await import("./core/embed.js");
...
async function search(engine, req, res) {
    const body = asObject(await readJsonBody(req));
    const query = typeof body['query'] === 'string' ? body['query'] : '';
    if (query.trim() === '')
        return sendError(res, 400, 'query is required');
    const kbIds = Array.isArray(body['kbIds']) ? body['kbIds'].filter((x) => typeof x === 'string') : undefined;
 ...
 */
export function mountKnowledgeRoutes(webServer, engine, listWorkspaces) {
    ...
    const handler = async (req, res) => {
        ...
        try {
            ...
            if (method === 'GET' && (tail === '' || tail === 'config'))
                return getConfig(engine, res);
            if (method === 'POST' && segments[0] === 'config')
                return postConfig(engine, req, res, segments[1] ?? '');
            if (method === 'GET' && tail === 'bases')
                return getBases(engine, res);
            if (method === 'POST' && tail === 'bases')
                return postBases(engine, req, res);
            if (method === 'POST' && tail === 'bases/delete')
                return deleteBase(engine, req, res);
            if (method === 'GET' && tail === 'files')
                return getFiles(engine, req, res);
            if (method === 'POST' && tail === 'files')
                return importFile(engine, req, res);
            if (method === 'POST' && tail === 'files/delete')
                return deleteFile(engine, req, res);
            if (method === 'GET' && tail === 'mounts')
                return getMounts(engine, res);
            if (method === 'POST' && tail === 'mounts')
                return setMount(engine, req, res);
            if (method === 'POST' && tail === 'mounts/delete')
                return deleteMount(engine, req, res);
            if (method === 'GET' && tail === 'workspaces')
                return getWorkspaces(listWorkspaces, res);
            if (method === 'POST' && tail === 'search')
                return search(engine, req, res);
            if (method === 'GET' && tail === 'health')
                return health(engine, res);
            if (method === 'GET' && tail === 'operations')
                return getOperations(engine, res);
            if (method === 'POST' && tail === 'operations/stop')
                return stopOperation(engine, req, res);
            if (method === 'POST' && tail === 'stop')
                return stopAllOperations(engine, req, res);
            if (method === 'POST' && tail === 'reindex')
                return reindexBase(engine, req, res);
            if (method === 'POST' && tail === 'reindex-all')
                return reindexAll(engine, req, res);
