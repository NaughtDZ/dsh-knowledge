 ...
 */
import { decodeText, normalizeLineEndings } from "./encoding.js";
import { isZip, readZipEntry, readZipByPrefix } from "./zip.js";
import { parseXml, descendants, textOf, localNameOf } from "./xml.js";
    ...
    while (i + 2 <= buf.length) {
        ...
        if (hi === 0 && ((lo >= 0x20 && lo < 0x7f) || (lo >= 0x80))) {
            ...
            while (j + 2 <= buf.length) {
                const l = buf[j];
                const h = buf[j + 1];
                if (h === 0 && (l >= 0x20 && l < 0x7f || l >= 0x80)) {
                    run += String.fromCharCode(l);
                    j += 2;
                }
                else
                    break;
    ...
    if (out.length === 0) {
        ...
        for (const byte of buf) {
            if (byte >= 0x20 && byte < 0x7f)
                ...
                run += String.fromCharCode(byte); // latin-1 letters
            else
                flush();
        }
        flush();
...
function htmlToText(html) {
    ...
    if (root === null)
        return normalizeLineEndings(html.replace(/<[^>]+>/gu, ' '));
    const blocks = [];
    const walk = (n, depth) => {
        for (const child of n.children) {
            if (child.kind === 'text') {
 ...
 */
export function extractText(filename, buffer, preferredEncoding, stripMarkup = true) {
    ...
    if (OLE2_EXT.has(lower) || isOle2(buffer)) {
        ...
        if (text.trim() === '' && isZip(buffer)) {
            ...
            if (tryText.text.trim() !== '')
                return { ...tryText, subtype: lower, degraded: true };
        }
...
function looksLikeTextLike(buf) {
    ...
    for (let i = 0; i < sample; i++) {
        ...
        if (b === 0)
            return false;
        if (b === 9 || b === 10 || b === 13 || (b >= 0x20 && b !== 0x7f))
            printable++;
    }
