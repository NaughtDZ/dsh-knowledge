 ...
 */
export async function embedTexts(config, inputs, signal) {
    if (inputs.length === 0)
        return [];
    ...
    const controller = signal !== undefined
        ? (() => { const c = new AbortController(); const onAbort = () => c.abort(); signal.addEventListener('abort', onAbort, { once: true }); return c; })()
        : timeoutController;
    let response;
    try {
    ...
    if (payload.error !== undefined)
        throw new Error(`embedding error: ${payload.error.message ?? 'unknown'}`);
...
export async function embedText(config, text, signal) {
...
export async function embedBatched(config, texts, maxCharsPerBatch = 8000, signal) {
    const vectors = [];
