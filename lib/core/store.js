...
export class KnowledgeStore {
    ...
    saveConfig(config) {
        ...
        this.db.prepare('INSERT INTO settings (id, json) VALUES (1, ?) ON CONFLICT(id) DO UPDATE SET json = excluded.json').run(JSON.stringify(next));
        return next;
    }
    // ── knowledge bases ────────────────────────────────────────────────────────
    createBase(name, description = '') {
        const now = Date.now();
        const id = randomUUID();
        this.db.prepare('INSERT INTO knowledge_bases (id, name, description, created_at, updated_at) VALUES (?, ?, ?, ?, ?)').run(id, name, description, now, now);
    ...
    baseOf(id) {
        const r = this.db.prepare('SELECT * FROM knowledge_bases WHERE id = ?').get(id);
        if (r === undefined)
            return null;
        const fileCount = Number(this.db.prepare('SELECT COUNT(*) AS n FROM files WHERE kb_id = ?').get(id).n);
        const chunkCount = Number(this.db.prepare('SELECT COUNT(*) AS n FROM chunks WHERE kb_id = ?').get(id).n);
        return {
            id: String(r.id), name: String(r.name), description: String(r.description ?? ''),
    ...
    addFile(f) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(row.id, row.kbId, row.name, row.ext, row.kind, row.relPath, row.size, row.encoding, row.lineEnding, row.charCount, row.chunkCount, row.status, row.error ?? null, row.embeddingModel ?? null, row.embeddingDim ?? null, row.createdAt, row.updatedAt);
        return row;
    }
    updateFile(id, patch) {
      ...
      WHERE id = ?
    `).run(next.ext, next.kind, next.relPath, next.size, next.encoding, next.lineEnding, next.charCount, next.chunkCount, next.status, next.error ?? null, next.embeddingModel ?? null, next.embeddingDim ?? null, next.updatedAt, id);
        return next;
    }
    fileOf(id) {
    ...
    fileOfRow(row) {
        return {
            id: row.id, kbId: row.kb_id, name: row.name, ext: row.ext, kind: row.kind,
            relPath: row.rel_path, size: row.size, encoding: row.encoding, lineEnding: row.line_ending,
            charCount: row.char_count, chunkCount: row.chunk_count, status: row.status,
    ...
    filesOf(kbId) {
        const rows = this.db.prepare('SELECT * FROM files WHERE kb_id = ? ORDER BY created_at DESC').all(kbId);
    ...
    loadChunks(kbIds) {
        ...
        const rows = this.db.prepare(`SELECT * FROM chunks WHERE kb_id IN (${placeholders})`).all(...kbIds);
    ...
    chunkIds(kbIds) {
        if (kbIds.length === 0)
            return [];
        const placeholders = kbIds.map(() => '?').join(',');
        const rows = this.db.prepare(`SELECT id FROM chunks WHERE kb_id IN (${placeholders})`).all(...kbIds);
        return rows.map(r => r.id);
    }
    fileNames() {
        const rows = this.db.prepare('SELECT id, name FROM files').all();
    ...
    workspaceKbIds(workspaceId) {
        const rows = this.db.prepare('SELECT kb_id FROM mounts WHERE workspace_id = ? AND enabled = 1').all(workspaceId);
        return rows.map(r => r.kb_id);
    }
    // ── search helpers ─────────────────────────────────────────────────────────
    /** Return candidate search hits (text + vector) for the given KBs. */
    searchCandidates(kbIds, limit = 10000) {
        if (kbIds.length === 0)
            return [];
        const placeholders = kbIds.map(() => '?').join(',');
        const rows = this.db.prepare(`
...
function rowVector(v) {
    if (v instanceof Uint8Array)
        return Array.from(new Float32Array(v.buffer.slice(0)));
    if (Array.isArray(v))
        return v;
    return [];
}
